...
```
평상시   → body-controller-state 만, 15초         VCSEC라 차가 자도 응답
깊은유휴 → 잠김+빈차+비충전이면 120초로 감속        차를 재우기 가장 좋은 상태
사건감지 → 차체 + 룰 카테고리 (+탑승 중이면 공조·배터리), 2초, 3분간
```

주기는 사이클 소요 시간을 빼고 잰다 — 읽기 실패(8초 타임아웃)가 껴도 간격이 일정하다.
충전 중은 깊은 유휴에서 제외한다: 차가 어차피 안 자고, 스텔스 충전이 신선한 값을 원한다.
