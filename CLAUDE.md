...
- UI는 **각지고 조용한 판**(0.8.31 확정, 사용자 시안 선택) — 그림자 없음·모서리 4dp·1dp 경계선·액센트는 검정(밤엔 흰색). 유채색은 의미색(냉방/난방/경보)에만. 그라데이션/글로우 금지, 값은 `T/Space/Radius/Motion` 토큰만
- **본문은 한 칸으로 세운다.** 좌우 2단은 사용자가 "정신없다"고 반려했다(0.8.31). 설정만 예외 — 서로 무관한 묶음이라 2단이 조용하게 읽힌다
