...
편집 탭은 큰 배경 면 대신 텍스트와 선택 밑줄로 탐색을 표시한다. 좌석 동작은 `좌석`과 `작동 단계`를 별도 라벨·24dp 구역 간격으로 구분한다. 좌석은 윤곽선 선택, 작동 값은 채움 선택으로 역할을 다르게 보이며 값은 `끄기·1단·2단·3단`으로 명시한다.
...
새 항목은 바로 펼쳐 편집하며, 탭 전환·화면 회전에서도 편집 위치를 유지한다. 이동은 같은 항목을 따라가고 삭제는 해당 편집기를 닫는다. 펼친 제목이 스크롤 밖으로 밀리지 않도록 배치 후 화면 안으로 이동한다. 요약 행은 최소 48dp이고 접근성에 편집/접힘 상태와 동작을 전달한다.
...
설정의 음성 연결(SmartThings) 아래에 Fleet API를 배치하며, 토큰 저장 후 입력칸과 저장 버튼은 숨기고 확인·삭제만 남긴다.
