...
설정은 주제별 카드로 나누고 값 선택은 공용 `ChoiceGrid`를 써 탐색 탭과 구별한다. 충전 상세는 실제 전류 범위·시간대, 명령 유효시간은 현재 초를 접힌 요약에 남긴다. 안심운전 실행 점검은 필요할 때 펼친다. 온라인 GPS 경로 전송과 수신 서버는 안내 스위치에 짧게 표시한다. 권한 요청, 점검 결과, 비밀값·백업 주의는 숨기지 않는다. 스마트싱스는 권한→명령 관리→유효시간 순서이며 명령 편집 시트를 유지한다.
켜면 부가 설정이 딸려 나오는 스위치(알림으로 차량 명령 실행·Fleet API·스텔스 충전과 시간대 제한·단속 카메라 안내·과속 경고음·카메라 접근 음성)는 `ExpandableToggle`로 기본 접힘이다. 접히면 제목 아래 현재 값 요약(예: `500m 전 안내 · 초과 +5km/h · 소리 띠링`)과 펼침 화살표만 보이고, 제목 줄 전체가 48dp 펼침 영역이다. 사용자가 꺼짐에서 켜는 순간에는 자동으로 펼치며, 백업 복원처럼 밖에서 켜진 값은 접은 채 둔다. 권한 부족·명령 취소 불가 주의·충전 진행 시간과 그래프·음성 엔진 상태·GPS 전송 고지는 접혀 있어도 계속 보인다. 바깥이 접혀 있으므로 안쪽의 전류 설정·명령 유효시간은 다시 접지 않는다.
