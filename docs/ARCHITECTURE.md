...
- 연결 수명은 `StatePoller`가 사용 모드·탑승 확인·앱 화면·단발 명령·실행 중 매크로를 합쳐 판정한다.
  휴대 모드는 앱 화면과 직접 명령을 허용하며, 자동 안심운전이 켜져 있으면 충전 상승 뒤
  최대 60초 동안 탑승을 확인한다. 확인은 VCSEC만 읽고 탑승 이벤트를 바로 보낸다.
  거치 모드는 자동 안심운전이 켜져 있으면 전원 상승 뒤 60초 확인 창 안에서 착석을 재확인하고,
  기존 매크로에 필요한 조회는 유지한다. 꺼져 있으면 첫 상태 확인으로 끝낸다.
  확인 종료 후 빈 차는 전원이 남아 있어도 GATT를 끊는다. 사용자의 즉시 해제는 다음 명시적 사용까지 우선한다.
- 스텔스 충전: `MacroService`가 `StealthChargeController`를 start/stop — 거치 모드는 1회 설정 동안 연결 보호를 잠시 미루고, 시간대 안에서 실제 상한의 위쪽 25% 전류를 조절한다. 충전 완료·수동 해제 뒤 원래 전류와 연결 보호 정책을 복구한다
