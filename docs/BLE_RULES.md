...
- **[MUST]** 기기 역할별로 인증 GATT 수명을 가른다 (`StatePoller.decideVehicleConnection`)
  - 개인 휴대폰: 앱 화면·직접 명령만 연결 사유다. 충전 전원·자동 매크로·보호 해제 저장값은 무시한다
  - 차량 태블릿: 전원 상승 뒤 첫 VCSEC 확인까지 연결하고, 탑승이 확인된 동안만 전원을 연결 사유로 쓴다.
    빈 차가 확인되면 앱 전면·직접 명령·실행 중 매크로·스텔스 충전 1회가 없는 즉시 끊는다.
...
| 기기 역할별 휴대폰 키 간섭 방지(빈 차 확인 후 GATT 종료) | ⚠️ 실차 미확인 — 태블릿/휴대폰 역할 설정 후 이탈 잠금 A/B 필요 |
