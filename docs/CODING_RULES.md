...
- **[NEVER]** 그라데이션·글로우를 쓰지 않는다. 휴대폰 우선 딥틸/세이지·쿨 뉴트럴·둥근 카드가 현행 방향(DESIGN.md)
...
- **다크는 0.8.22부터 허용**. 단 "다크 테마"가 아니라 **시계 기준 낮/밤 자동 전환**이다
  ...
  - 순검정 금지 — 어두운 녹색 배경(`#101A17`). 야간 눈부심·잔상 때문
...
- **제어 화면의 색 규칙**: 정상은 `T.Ok`, 선택·주요 동작은 `T.Electric`. `TileTone.Cool/Warm` = 차가 실제로 일하는 중, `TileTone.Alert` = 사람이 봐야 함(면 전체가 물듦)
  - why: 색이 곧 "이걸 봐라"라는 신호다. 정상·선택 상태는 절제된 강조를 쓰고 경보와 구별한다
...
- 스냅샷 테스트는 `TeslaMacroTheme(dark = …)`로 낮/밤을 **못 박는다**. 자동 판정에 맡기면 실행 시각에 따라 결과가 바뀐다
  - ✅ `app/src/main/java/com/wemade/teslamacro/ui/theme/Color.kt:20` `Void = 0xFFF3F6F5` (쿨 뉴트럴) + 흰 카드
