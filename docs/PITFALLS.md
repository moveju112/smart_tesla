...
- **Symptom:** 페어링된 기기 목록의 테슬라 주소로 직접 연결하면 30초 타임아웃만 반복 (실차 확인 2026-08-13)
  ...
  - Fix: nRF Connect에서 `S<VIN해시>C` 이름을 찾아 키 연결용 BLE 주소를 확인한다
...
- **Symptom:** 실기기에 APK가 설치 안 되거나 universal APK를 못 찾음
  - Cause: ABI split만 있고 universal 미생성
...
**0.9.35 대응**: `키가드잠금=true` 또는 `기기잠금=true`이면 내부 인증 Activity에서 화면을 켜고 시스템 잠금 해제를 요청한다.
두 값이 모두 false인 것을 콜백·실행 이어가기·외부 전달 직전에 확인한다.
같은 안심운전 URI를 Activity가 살아 있는 동안 전달하며 신뢰 기기 등록은 요구하지 않는다.
0.9.32는 `기기잠금`만 검사해 키가드가 남은 상태에서도 배경 직접 실행으로 빠졌다.
실차 09-11 00:09에는 화면 꺼짐·키가드=true·기기잠금=false에서 전달 로그만 남고 네이버가 표시되지 않았다.
분기 누락은 코드로 확인했지만 OS 차단과 네이버 내부 종료 중 실제 원인은 기기 로그로 확정되지 않았다.
인증 취소·60초 만료·프로세스 재시작 뒤 오래된 알림은 실행하지 않는다.
인증 화면의 자동 표시가 지연되면 같은 요청의 알림에서 이어갈 수 있다(알림 허용 필요).
고정 3초 홈 전환과 해당 설정은 제거했다. 시스템 전달 로그는 여전히 실제 안심운전 시작 증거가 아니다.
단위 테스트는 요청 수명, 스냅샷은 안내 화면을 검증하며 삼성 기기 인증·네이버 실제 시작은 실기기 확인 대상이다.

**0.9.31 점검**: 설정 → 주행에서 실행 방식을 고르고 `10초 뒤 실행 점검`을 누른 뒤
화면을 잠근다. 예약은 서비스가 유지하며 전체 진단은 두 통로를 순서대로 호출한다.
점검은 홈 전환을 생략하고 화면 켜짐·잠금·대상 앱 버전을 실행 시각과 함께 기록한다.
전체 진단에서 관찰한 성공은 다음 점검에서 각 통로를 단독 실행해 구분한다.
실제 탑승 자동 실행은 기존대로 한 통로만 호출하며, BLE 확인과 실행 전달 로그를 따로 본다.
...
**Android 14+ 추가 대응 (0.9.18)**: 보이는 오버레이 위에서 `PendingIntent`를 보내고,
송신자·생성자의 background activity start 모드를 `ALLOWED`로 명시한다.
resolve된 대상 Activity와 전달 방식을 로그에 남기되, 시스템 전달을 화면 표시 성공으로 부르지 않는다.
