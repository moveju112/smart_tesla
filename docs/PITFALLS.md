...
**증상**: 매크로가 정상 발동하고 진단 로그에 `네이버 지도 안내 시작 → …`까지 찍히는데 지도가 안 뜬다.
앱을 한 번 열어보면 그때는 뜬다.
...
**대응 (0.8.32)**: `NaverNavigator.launchFromBackground()` — 1×1 투명 창을 잠깐 올렸다가
인텐트를 던지고 곧바로 내린다. 창 조작은 메인 스레드에서만 되고, 붙는 데 한 프레임이 걸려 짧게 기다린다.
창을 못 올리면 `지도 안내 · 오버레이 창 실패` 를 로그에 남긴다.
