...
- 별도 동의 시 도로 위치 보정만 제공한다. 단속 방향 확정·이동식 카메라·전체 도로 제한속도·구간 평균속도 판정은 제공하지 않는다.
...
- `설정 → 주행 → 주변 카메라 도로 매칭`은 기본 꺼짐, 백업 복원 시에도 꺼짐. 안내 동의만으로 위치를 서버에 전송하지 않는다.
- 주행 중 저장된 카메라 약 1km 내의 넓은 전방 후보(방향 차 120° 이내, 100m 이내는 후방도 허용)에서만 최근 GPS 2~8개를 요청당 최소 10초 간격으로 `POST https://gps-map.choondoggy.com/v1/match` 본문에 전송한다. 후보 이탈·설정 변경·안내 재시작에도 429/503 대기를 포함한 요청 간격을 유지한다(앱 프로세스 재시작은 제외). 지도 전용 토큰은 Git 제외 `local.properties`의 `roadMatchToken`을 빌드에 주입한다. 없는 빌드는 오프라인으로만 동작한다. **APK에 주입된 토큰은 모든 사용자에게 공유되고 추출 가능하다**. 공개 배포 전 토큰 발급·회전·요청률 제한 정책을 확인한다.
