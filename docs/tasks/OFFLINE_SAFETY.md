...
- 주행 중 저장된 카메라 약 1km 내에서만 최근 GPS 2~8개를 최대 10초에 한 번 `POST https://gps-map.choondoggy.com/v1/match` 본문으로 전송한다. 지도 전용 토큰은 Git 제외 `local.properties`의 `roadMatchToken`을 빌드에 주입한다. 없는 빌드는 오프라인으로만 동작한다. **APK에 주입된 토큰은 모든 사용자에게 공유되고 추출 가능하다**. 공개 배포 전 토큰 발급·회전·요청률 제한 정책을 확인한다.
- `matched`·미매칭 0·신뢰도 0.8 이상인 경로의 끝점만 30m/5초 이내에 GPS 위치 보조로 사용한다. `uncertain`/실패/지연/토큰 오류는 오프라인 GPS로 되돌린다. 카메라 자료에 단속 방향·도로 ID가 없으므로 매칭도 정확한 단속 도로 확인이 아니다.
